...
interface TaskRepository {
    ...
    suspend fun getTasks(states: List<TaskState>, courseIds: List<Int>): List<TaskSummary>

    fun observeTasks(
        states: List<TaskState> = emptyList(),
        courseIds: List<Int> = emptyList(),
        from: Instant,
        to: Instant
     ...
     */
    context(raise: Raise<TaskError>)
    suspend fun getCourseExercises(courseId: Int): CourseWithExercises
     ...
     */
    context(raise: Raise<TaskError>)
    suspend fun prolongDeadline(taskId: Int, lateDays: Int)
}